    ...
    public synchronized String getMessage() throws LLPException, IOException 
    {
        ...
        boolean end_of_message = false;
            ...
            try {
                ...
                Log.d(Constants.TAG, "SocketTimeoutException on read() attempt.");
                return null;               
            } catch (SocketException e) {
                Log.i(Constants.TAG, "SocketException on read() attempt.  Socket appears to have been closed: " + e.getMessage());
                throw e;
            }

            //trying to read when there is no data (stream may have been closed at other end)
            if (c == -1) {
                Log.i(Constants.TAG, "End of input stream reached.");
