...
public class MessageIDGenerator {
     ...
     */
    //public final static String DEFAULT_ID_FILE = Home.getHomeDirectory().getAbsolutePath() + "/id_file";//TODO : original
    public final static String DEFAULT_ID_FILE = Environment.getExternalStorageDirectory() + "/id_file";
    ...
    void initialize() throws IOException {
        ...
        try{
            ...
            if (extFile.createNewFile()== true){
                ...
                if (e <= 0){
                    ...
                    try {
                    	...
                    	Log.w(Constants.TAG, "Failed to parse message ID file value \"" + idStrTrim + "\". Defaulting to 0.");
                    }
            ...
            Log.e(Constants.TAG, "Failed to locate message ID file. Message was: {"+e.getMessage()+"}");
        } catch (IOException e) {
        	if (Boolean.TRUE.equals(System.getProperty(NEVER_FAIL_PROPERTY, Boolean.TRUE.toString()))) {
        		Log.w(Constants.TAG, "Could not retrieve message ID file, going to default to ID of 0. Message was: {"+e.getMessage()+"}");
        		id = 0;
        		return;
        	} else {
        		throw e;
    ...
    public synchronized String getNewID() throws IOException{
    	try {
        	...
        	if (Boolean.TRUE.equals(System.getProperty(NEVER_FAIL_PROPERTY, Boolean.TRUE.toString()))) {
        		Log.i(Constants.TAG, "Failed to create message ID file. Message was: {"+e.getMessage()+"}");
        		fileW = null;
        	}
    	} catch (IOException e) {
        	if (Boolean.TRUE.equals(System.getProperty(NEVER_FAIL_PROPERTY, Boolean.TRUE.toString()))) {
        		Log.d(Constants.TAG, "Failed to create message ID file. Message was: {"+e.getMessage()+"}");
        		fileW = null;
        	} else {
        		throw e;
        	}
