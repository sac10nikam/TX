...
public class DefaultModelClassFactory implements ModelClassFactory {
    ...
    private static Class<?> findClassInASpecificPackage(String name, String version, String type, String packageName) throws HL7Exception { 
		...
		try { 
			...
			Log.d(Constants.TAG, "Unable to find class " + classNameToTry + ", using default");
			return findClass(name, version, type); 
		} 
    ...
    private static String[] loadPackages(String version) throws HL7Exception {
        ...
        if ( resourceInputStream != null) {
            ...
            try {
                ...
                while (line != null) {
                    Log.i(Constants.TAG, "Adding package to user-defined package list: {"+line+"}");
                    packageList.add( line );
                    line = in.readLine();
                }
                
            } catch (IOException e) {
                Log.e(Constants.TAG, "Can't load all the custom package list - user-defined classes may not be recognized");
            }
            
        }
        else {
            Log.d(Constants.TAG, "No user-defined packages for version {"+version+"}");
        }
    ...
    private static Class<?> findClass(String name, String version, String type) throws HL7Exception {
        ...
        while (compClass == null && c < packageList.length) {
            ...
            try {
                ...
                Log.d(Constants.TAG, "Trying to load: {"+classNameToTry+"}");                    
                compClass = Class.forName(classNameToTry);
                Log.d(Constants.TAG, "Loaded: {"+classNameToTry+"} class: {"+compClass+"}");                    

            }
            catch (ClassNotFoundException cne) {
                Log.d(Constants.TAG, "Failed to load: {"+classNameToTry+"}");                    
                /* just try next one */
            }
            c++;
        }
