...
public class MessageIterator implements java.util.Iterator<Structure> {
    ...
    private List<Position> popUntilMatchFound(List<Position> theDefinitionPath) {
        ...
        if (newCurrentStructureDefinition.isFinalChildOfParent()) {
            if (theDefinitionPath.size() > 1) {
            	...
            	Log.d(Constants.TAG, "Popped to root of message and did not find a match for {"+myDirection+"}");
                return null;
            }
        }
    ...
    public boolean hasNext() {
        ...
        Log.d(Constants.TAG, "hasNext() for direction {"+myDirection+"}");
        if (myDirection == null) {
        ...
        while (!myNextIsSet) {
            ...
            Log.d(Constants.TAG, "hasNext() current position: {"+currentPosition+"}");
    ...
    private void addNonStandardSegmentAtCurrentPosition() throws Error {
    	Log.d(Constants.TAG, "Creating non standard segment {"+myDirection+"} on group: {"+getCurrentPosition().getStructureDefinition().getParent().getName()+"}");
